...
public class ServletUsuario extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request,
		...
		if (acao != null && acao.equalsIgnoreCase("reset")) {
			...
			usuario.setId((id != null && !id.isEmpty()) ? Long.parseLong(id)
					: null);
			...
			try {
				...
				if (ServletFileUpload.isMultipartContent(request)) {
					...
					if (imagemFoto != null
							&& imagemFoto.getInputStream().available() > 0) {
						...
						byte[] imageByteDecode = new Base64()
								.decodeBase64(fotoBase64);
						BufferedImage bufferedImage = ImageIO
								.read(new ByteArrayInputStream(imageByteDecode));
						...
						BufferedImage resizedImage = new BufferedImage(100,
								100, type);
					...
					if (curriculoPdf != null
						...
						String curriculoBase64 = new Base64()
								.encodeBase64String(converteStremParabyte(curriculoPdf
										.getInputStream()));
				...
				if (login == null || login.isEmpty()) {
					...
					msg = "O sexo dever ser informado";
					podeInserir = false;
					...
					request.setAttribute("msg",
							"Esta Senha Pertence a Um Usuário!");
					podeInserir = false;
				}
				...
				if (id != null && !id.isEmpty() && podeInserir) {
					if (!daoUsuario.validarLoginUpdate(login, id)) {
						request.setAttribute("msg",
								"Login já existe para outro usuário");
					} else {
